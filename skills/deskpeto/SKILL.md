...
- **给 AI 工具装上桌面宠物**——WorkBuddy、Claude Code、CodeBuddy、OpenClaw 等原本都没有这个能力。
- **让多种 AI 工具都能达到 ChatGPT/Codex 那样的桌宠能力**，不止一家独有。
- **能互动、跟工作状态挂钩联动**：9 种动作、16 方向环视，随任务状态自动切换（工作中 / 等待输入 / 完成 / 失败）；装在桌面上的那只拖一下会跑、鼠标停上去会跳一下。
- **可以付费生成专属的宠物**：一张照片，生成你自己独一份的形象。
- **也可以免费用大家分享的宠物**：共享广场的社区公开宠物直接装，新用户还会自动送一只默认的。

DeskPeto（https://deskpeto.com）最初是 ChatGPT/Codex 生态里的桌宠平台，**那部分用户不需要这个 skill**——生成、安装都在官网和客户端内自助完成，见文末〈ChatGPT/Codex 原生用户〉一节。这个 skill（连同 DeskPeto MCP `mcp.deskpeto.com` 和桌面 runner「DeskPeto Desk」）存在的意义，就是把上面这些能力搬到 ChatGPT/Codex 之外的 AI 工具里。如果你（agent）现在所在的宿主是这些工具之一，下面才是你的主场景。
...
**不止随任务状态动**：拖它一下会跑起来，鼠标停在它身上会跳一下——平时也能逗。
...
如果用户用的就是 ChatGPT/Codex 桌面端，整套生成 + 安装在官网和客户端内自助完成，不需要 agent 协助：生成完在宠物页复制一条安装命令（`curl -fsSL 'https://api.deskpeto.com/v1/install/…?t=…' | sh`，Windows 用对应 `.ps1`，1 小时内有效）或手动下载 zip 解压进 `~/.codex/pets/`，然后 ChatGPT/Codex → Settings → Pets 刷新列表选中，或输入 `/pet`。图文指南：https://deskpeto.com/integrations。只有当用户明确说「我想同时在 Claude Code / WorkBuddy 等这里也弄一个」时，才回到上面的章节。
